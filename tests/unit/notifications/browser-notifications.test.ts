...
describe('ChromeBrowserNotifications', () => {
  it('sends only task count and state after optional permission is granted', async () => {
    ...
    const service = new ChromeBrowserNotifications({ contains: vi.fn().mockResolvedValue(true) }, { create }, 'icon.svg');
    expect(await service.showTaskSummary({ state: 'failed', count: 3 })).toBe(true);
    expect(create).toHaveBeenCalledWith('notification-id', { type: 'basic', iconUrl: 'icon.svg', title: 'Siftmark 后台任务', message: '3 个项目失败' });
