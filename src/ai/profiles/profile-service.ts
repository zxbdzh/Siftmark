...
export class ModelProfileService {
  ...
  async verify(
  ...
  ): Promise<{ profile: ModelProfile; probe: CapabilityProbe }> {
    ...
    const requiresText = profile.capabilities.some(
      (capability) => capability !== 'embed'
    );
    const requiresEmbedding = profile.capabilities.includes('embed');
    if (
      !probe.authentication ||
      (requiresText && (!probe.text || !probe.structuredOutput)) ||
      (requiresEmbedding && !probe.embedding)
    )
      throw new Error('模型未通过所需能力验证');
    const verified = await this.profiles.put({
      ...profile,
      state: 'verified',
      verifiedAt: Date.now()
    });
  ...
  getAssignments(): Promise<ProfileAssignments> {
