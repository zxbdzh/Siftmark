...
export function parseProbeText(text: string): void {
  try {
    ...
    if (value.ok !== true || Object.keys(value).some((key) => key !== 'ok')) throw new Error('invalid probe');
  } catch {
    throw new ProviderError('validation', 'Provider did not return the required structured probe result');
  }
}
