...
export class ReviewService {
  ...
  async applyProposal(input: { proposalId: string; fields: Array<'title' | 'folder' | 'tags' | 'summary'> }): Promise<void> {
    ...
    if (input.fields.includes('title')) await this.commands.rename({ bookmarkId: proposal.bookmarkId, title: proposal.result.title, expectedTitle: proposal.sourceSnapshot.title });
    if (input.fields.includes('folder')) {
      const folderId = proposal.result.folderPath.at(-1);
      if (folderId) await this.commands.move({ bookmarkId: proposal.bookmarkId, parentId: folderId, expected: { parentId: proposal.sourceSnapshot.parentId, index: proposal.sourceSnapshot.index } });
    }
