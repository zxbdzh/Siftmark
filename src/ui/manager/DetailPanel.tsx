...
const emptyMetadata = (bookmarkId: string): BookmarkMetadata => ({
  bookmarkId,
  summary: '',
  tags: [],
  note: '',
...
export interface DetailLifecycleAction {
  label: string;
...
export function DetailPanel({
  ...
  useEffect(() => {
    ...
    if (metadataRepository)
      void metadataRepository
        .get(bookmark.id)
        .then((value) => setMetadata(value ?? emptyMetadata(bookmark.id)));
  ...
  const save = async () => {
    if (commands) {
      ...
      if (!renamed.ok) {
        setStatus('标题已在别处更新，请刷新后重试');
        return;
      }
      if (metadata && metadataRepository)
        await commands.updateMetadata({ ...metadata, updatedAt: Date.now() });
    ...
    >
      {thumbnailRepository ? (
        <ThumbnailPreview
          ...
          onRefresh={() => onRefreshThumbnail?.(bookmark)}
        />
      ) : null}
      <label>
        标题
      ...
      {metadata ? (
        <>
          ...
          <MarkdownNoteEditor
            initialValue={metadata.note}
            onSave={(note) => setMetadata({ ...metadata, note })}
          />
          <dl className="detail-status">
            <div>
              <dt>AI 置信度</dt>
      ...
      {restoreAction ? (
        <div className="detail-lifecycle-actions">
          <button type="button" onClick={restoreAction.onAction}>
            <RotateCcw size={16} />
            {restoreAction.label}
          </button>
        </div>
      ) : archiveAction || recycleAction ? (
        <div className="detail-lifecycle-actions">
          {archiveAction ? (
            <button
              type="button"
              onClick={archiveAction.onAction}
              disabled={Boolean(archiveAction.disabledReason)}
              title={archiveAction.disabledReason}
            >
              <Archive size={16} />
              {archiveAction.label}
            </button>
          ...
          {recycleAction ? (
            <button
              type="button"
              onClick={recycleAction.onAction}
              disabled={Boolean(recycleAction.disabledReason)}
              title={recycleAction.disabledReason}
            >
              <Trash2 size={16} />
              {recycleAction.label}
            </button>
