...
export function ManagerLayout({ nodes, loading, repository, commands, recycleBinId, onRefresh, onAnalyze, onHealthScan, reviewWorkspace, notificationCenter, usageInsights, metadataRepository, metadataById = new Map(), visitsById = new Map(), sortRepository, draftWorkspace, thumbnailRepository, searchService, onRefreshThumbnail }: ManagerLayoutProps) {
  const [drawer, setDrawer] = useState<'folders' | 'detail' | null>(null);
  const [view, setView] = useState<'bookmarks' | 'review' | 'drafts' | 'notifications' | 'insights'>('bookmarks');
  ...
  const searchRank = useMemo(() => searchResults ? new Map(searchResults.map((result, index) => [result.bookmarkId, index])) : undefined, [searchResults]);
  const bookmarks = useMemo(() => {
    const matches = nodes.filter((node) => isBookmark(node) && (searchRank ? searchRank.has(node.id) : (!folderId || node.parentId === folderId)));
    if (searchRank) return matches.sort((left, right) => (searchRank.get(left.id) ?? Number.MAX_SAFE_INTEGER) - (searchRank.get(right.id) ?? Number.MAX_SAFE_INTEGER));
    return sortBookmarks(matches, sort, metadataById, visitsById);
  }, [folderId, metadataById, nodes, searchRank, sort, visitsById]);
  ...
  const handleSelectSearchResult = useCallback((bookmarkId: string) => useManagerStore.getState().selectBookmark(bookmarkId), []);
