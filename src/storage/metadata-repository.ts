...
export class DexieMetadataRepository implements MetadataRepository {
  ...
  get(bookmarkId: string): Promise<BookmarkMetadata | null> {
    return this.db.bookmarkMetadata.get(bookmarkId).then((value) => value ?? null);
  }

  async put(metadata: BookmarkMetadata): Promise<void> {
    await this.db.transaction('rw', this.db.bookmarkMetadata, this.db.softDeletedMetadata, async () => {
  ...
  async softDelete(bookmarkId: string, deletedAt: number): Promise<void> {
    await this.db.transaction('rw', this.db.bookmarkMetadata, this.db.softDeletedMetadata, async () => {
      ...
      if (!metadata) return;
      await this.db.softDeletedMetadata.put({ bookmarkId, payload: metadata, deletedAt });
  ...
  async purgeDeletedBefore(timestamp: number): Promise<number> {
    return this.db.transaction('rw', this.db.softDeletedMetadata, async () => {
      const rows = await this.db.softDeletedMetadata.where('deletedAt').below(timestamp).toArray();
      if (rows.length > 0) await this.db.softDeletedMetadata.bulkDelete(rows.map((row) => row.bookmarkId));
      return rows.length;
    });
  }
}
