...
export class LinkChecker {
  ...
  async check(url: string, signal?: AbortSignal): Promise<LinkHealth> {
    try {
      let response = await this.fetcher.call(globalThis, url, {
        method: 'HEAD',
        redirect: 'follow',
        signal
      });
      ...
      return {
        ...
        redirected:
          response.redirected || Boolean(response.url && response.url !== url),
        checkedAt: this.now()
      };
    } catch (error) {
      ...
      return {
        url,
        status: isBlockedError(error) ? 'blocked' : 'temporary',
        redirected: false,
        checkedAt: this.now()
      };
    }
  ...
  async checkMany(
    ...
    for (const url of urls) {
      ...
      groups.set(domain, [...(groups.get(domain) ?? []), url]);
    }
    const results = await Promise.all(
      [...groups.values()].map((group) =>
        runWithConcurrency(
          group,
          Math.max(1, options.concurrencyPerDomain ?? 2),
          (url) => this.check(url, options.signal),
          options.signal
        )
      )
    );
