...
export interface SearchDocumentRepository {
  ...
  putDocuments(documents: SearchDocument[]): Promise<void>;
  deleteDocuments(bookmarkIds: string[]): Promise<void>;
}
  ...
  private async performSync(documents: SearchDocument[]): Promise<void> {
    if (!this.initialized) {
      ...
      this.signatures = new Map(
        persisted.map((document) => [
          document.bookmarkId,
          signatureOf(document)
        ])
      );
    ...
    const next = new Map(
      documents.map((document) => [document.bookmarkId, document])
    );
    const changed = documents.filter(
      (document) =>
        this.signatures.get(document.bookmarkId) !== signatureOf(document)
    );
    const removed = [...this.signatures.keys()].filter(
      (bookmarkId) => !next.has(bookmarkId)
    );
    ...
    await Promise.all([
      changed.length
        ? this.repository.putDocuments(changed)
        : Promise.resolve(),
      removed.length
        ? this.repository.deleteDocuments(removed)
        : Promise.resolve()
    ]);

    this.signatures = new Map(
      documents.map((document) => [document.bookmarkId, signatureOf(document)])
    );
  }
}
