...
import { createCaptureThumbnailHandler, type CaptureThumbnailInput } from '../src/tasks/handlers/capture-thumbnail';
...
import { createIndexEmbeddingsHandler, type IndexEmbeddingsInput } from '../src/tasks/handlers/index-embeddings';
...
import { createScanHealthHandler, type ScanHealthInput } from '../src/tasks/handlers/scan-health';
import { ChromeHealthScheduler, HEALTH_SCAN_ALARM, type HealthSchedule } from '../src/platform/chrome/scheduler';
...
export default defineBackground(() => {
  ...
  const thumbnailService = new ThumbnailService({ captureVisibleTab: (windowId, options) => browser.tabs.captureVisibleTab(windowId, options) }, thumbnailRepository);
  const embeddingIndexer = new EmbeddingIndexer(new EmbeddingRepository(database));
  const healthScanService = new HealthScanService(new LinkChecker(), metadata, proposals);
  const healthScheduler = new ChromeHealthScheduler(browser.alarms, browser.storage.local);
  const notificationService = new NotificationService(new NotificationRepository(database), new ChromeBrowserNotifications(browser.permissions, browser.notifications, notificationIcon));
  ...
  const enqueueThumbnail = async (input: { bookmarkId: string; tabId?: number; windowId?: number }) => {
    const now = Date.now();
    const taskId = crypto.randomUUID();
    await tasks.put({ id: taskId, type: 'capture-thumbnail', state: 'queued', input, completed: 0, failed: 0, retryCount: 0, idempotencyKey: crypto.randomUUID(), createdAt: now, updatedAt: now });
    void runner.runNext();
    return { taskId };
  };
  const enqueueConfiguredEmbeddings = async (bookmarkIds?: string[]) => {
    ...
    if (existing && ['queued', 'running', 'paused'].includes(existing.state)) return { taskId };
    const now = Date.now();
    const input: IndexEmbeddingsInput = { profileId, profileVersion, ...(bookmarkIds ? { bookmarkIds } : {}) };
    await tasks.put({ id: taskId, type: 'index-embeddings', state: 'queued', input, profileVersion, completed: 0, failed: 0, retryCount: 0, idempotencyKey: `embedding:${assignment}`, createdAt: existing?.createdAt ?? now, updatedAt: now });
    void runner.runNext();
    return { taskId };
  };
  const enqueueHealthScan = async (input: ScanHealthInput = {}) => {
    const now = Date.now();
    const taskId = crypto.randomUUID();
    await tasks.put({ id: taskId, type: 'scan-health', state: 'queued', input, completed: 0, failed: 0, retryCount: 0, idempotencyKey: crypto.randomUUID(), createdAt: now, updatedAt: now });
    void runner.runNext();
    return { taskId };
  };
  const saveService = new SaveService(bookmarks, { enqueue: enqueueAnalysis });
  const saveActiveTab = async () => { const [tab] = await browser.tabs.query({ active: true, currentWindow: true }); if (!tab) return undefined; const result = await saveService.saveCurrentTab(tab); if (result.bookmarkId) void enqueueThumbnail({ bookmarkId: result.bookmarkId, tabId: tab.id, windowId: tab.windowId }); return result; };
  ...
  runner.register('capture-thumbnail', createCaptureThumbnailHandler(thumbnailService, async (input: CaptureThumbnailInput) => {
    const bookmark = await bookmarks.get(input.bookmarkId);
    const requestedTab = input.tabId === undefined ? undefined : await browser.tabs.get(input.tabId).catch(() => undefined);
    const [activeTab] = await browser.tabs.query({ active: true, ...(requestedTab?.windowId !== undefined ? { windowId: requestedTab.windowId } : { currentWindow: true }) });
    const tab = requestedTab ?? activeTab;
    const sameTab = Boolean(tab?.id !== undefined && activeTab?.id === tab.id);
    const sameUrl = Boolean(bookmark?.url && tab?.url === bookmark.url);
    let capture: PageCapture | undefined;
    if (sameTab && sameUrl && tab?.id !== undefined) capture = await browser.tabs.sendMessage(tab.id, { type: 'capture-page' }).catch(() => undefined) as PageCapture | undefined;
    return { ...input, tabId: tab?.id, activeTabId: activeTab?.id, windowId: tab?.windowId, screenshotAllowed: sameTab && sameUrl && capture?.policy.screenshot === 'allowed' };
  }));
  runner.register('index-embeddings', createIndexEmbeddingsHandler({
    profiles,
    adapters,
    indexer: embeddingIndexer,
    loadDocuments: async (bookmarkIds) => {
      const [nodes, metadataRows, visitRows] = await Promise.all([bookmarks.getTree(), database.bookmarkMetadata.toArray(), database.visitAggregates.toArray()]);
      const filter = bookmarkIds ? new Set(bookmarkIds) : undefined;
      return buildSearchDocuments(nodes, new Map(metadataRows.map((row) => [row.bookmarkId, row])), new Map(visitRows.map((row) => [row.bookmarkId, row.lastVisitedAt ?? 0]))).filter((document) => !filter || filter.has(document.bookmarkId));
    }
  }));
  const scanHealthHandler = createScanHealthHandler(healthScanService, () => bookmarks.getTree());
  runner.register<ScanHealthInput>('scan-health', async (context) => {
    const result = await scanHealthHandler(context);
    if (result.state === 'succeeded' || result.state === 'paused' || result.state === 'failed') {
      await notificationService.notify({ type: result.state === 'succeeded' ? 'task-succeeded' : result.state === 'paused' ? 'task-paused' : 'task-failed', title: '健康检查', message: `${result.completed ?? context.task.completed} 个项目${result.state === 'succeeded' ? '已检查' : result.state === 'paused' ? '等待继续' : '检查失败'}`, taskId: context.task.id, browserSummary: { state: result.state, count: result.completed ?? context.task.completed } });
      void browser.runtime.sendMessage({ type: 'notifications-changed' }).catch(() => undefined);
    }
    return result;
  });
  ...
  browser.runtime.onMessage.addListener((message: unknown) => {
    const value = message as { type?: string; input?: AnalyzeTaskInput & { windowId?: number; folderId?: string } };
    if (value.type === 'queue-analysis' && value.input) return enqueueAnalysis(value.input);
    if (value.type === 'queue-thumbnail' && value.input) return enqueueThumbnail(value.input);
    if (value.type === 'queue-embeddings') return enqueueConfiguredEmbeddings(value.input?.bookmarkId ? [value.input.bookmarkId] : undefined);
    if (value.type === 'queue-health-scan') return enqueueHealthScan(value.input?.folderId ? { folderId: value.input.folderId } : {});
    if (value.type === 'configure-health-schedule' && value.input) return healthScheduler.configure(value.input as unknown as HealthSchedule);
  ...
  browser.tabs.onUpdated.addListener((_tabId, changeInfo, tab) => {
    ...
    void browser.storage.local.get(VISIT_TRACKING_KEY).then(async (stored) => {
      ...
      const matches = (await bookmarks.getTree()).filter((node) => node.url && normalizeUrlConservatively(node.url) === target);
      await Promise.all(matches.map((bookmark) => visitAggregator.record(bookmark.id, Date.now())));
      if (matches.length > 0) void browser.runtime.sendMessage({ type: 'visits-changed' }).catch(() => undefined);
    });
  });
  browser.alarms.onAlarm.addListener((alarm) => {
    if (alarm.name === TASK_WAKE_ALARM) void runner.runNext();
    else if (alarm.name === HEALTH_SCAN_ALARM) void healthScheduler.getSchedule().then(async (schedule) => {
      if (!schedule.enabled) return;
      if (schedule.folderIds.length === 0) await enqueueHealthScan();
      else for (const folderId of schedule.folderIds) await enqueueHealthScan({ folderId });
    });
  });
});
