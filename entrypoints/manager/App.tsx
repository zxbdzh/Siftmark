...
export default function App() {
  ...
  const thumbnails = useMemo(() => new DexieThumbnailRepository(database), [database]);
  const localSearchIndex = useMemo(() => new LocalSearchIndex(), []);
  const searchIndexRepository = useMemo(() => new SearchIndexRepository(database), [database]);
  const searchSynchronizer = useMemo(() => new SearchIndexSynchronizer(localSearchIndex, searchIndexRepository), [localSearchIndex, searchIndexRepository]);
  const embeddingRepository = useMemo(() => new EmbeddingRepository(database), [database]);
  const vectorSearch = useMemo(() => new VectorSearch(embeddingRepository), [embeddingRepository]);
  const notificationRepository = useMemo(() => new NotificationRepository(database), [database]);
  ...
  const searchDocuments = useMemo(() => buildSearchDocuments(nodes, metadataById, visitsById), [metadataById, nodes, visitsById]);
  const semanticSearch = useMemo(() => {
    if (!embeddingProfile) return undefined;
    const adapter = adapters.get(embeddingProfile.protocol);
    return adapter?.embed ? new EmbeddingSemanticSearch(embeddingProfile, adapter, vectorSearch, () => searchDocuments) : undefined;
  }, [adapters, embeddingProfile, searchDocuments, vectorSearch]);
  const searchService = useMemo(() => new SearchService(localSearchIndex, semanticSearch), [localSearchIndex, semanticSearch]);
  ...
  const refreshNotifications = useCallback(async () => setNotifications(await notificationRepository.list()), [notificationRepository]);
  useEffect(() => { void Promise.all([refresh(), refreshProposals(), refreshNotifications(), noteDraftRepository.list().then(setNoteDrafts), hydrateTheme(settings), settings.getSpecialFolders().then((value) => setRecycleBinId(value.recycleBinId))]).finally(() => setLoading(false)); }, [noteDraftRepository, refresh, refreshNotifications, refreshProposals, settings]);
  useEffect(() => { const listener = (message: unknown) => { const type = (message as { type?: string }).type; if (type === 'notifications-changed') void Promise.all([refresh(), refreshNotifications(), refreshProposals()]); else if (type === 'visits-changed') void refresh(); }; browser.runtime.onMessage.addListener(listener); return () => browser.runtime.onMessage.removeListener(listener); }, [refresh, refreshNotifications, refreshProposals]);
  useEffect(() => { void settings.getProfileAssignments().then(async (assignments) => { const key = assignments.embed; if (!key) return; const separator = key.lastIndexOf('@'); if (separator <= 0) return; const profile = await profiles.get(key.slice(0, separator), key.slice(separator + 1)); if (profile?.state === 'verified' && profile.capabilities.includes('embed')) setEmbeddingProfile(profile); }); }, [profiles, settings]);
  useEffect(() => { if (!loading) void searchSynchronizer.sync(searchDocuments).catch((error: unknown) => console.error('搜索索引同步失败', error)); }, [loading, searchDocuments, searchSynchronizer]);
  return <ManagerLayout nodes={nodes} loading={loading} repository={repository} commands={commands} metadataRepository={metadata} metadataById={metadataById} visitsById={visitsById} thumbnailRepository={thumbnails} searchService={searchService} onRefreshThumbnail={(bookmark) => void browser.runtime.sendMessage({ type: 'queue-thumbnail', input: { bookmarkId: bookmark.id } })} sortRepository={settings} recycleBinId={recycleBinId} onRefresh={refresh} onAnalyze={(bookmark) => void browser.runtime.sendMessage({ type: 'queue-analysis', input: { bookmarkId: bookmark.id } }).then(() => globalThis.setTimeout(() => void refreshProposals(), 300))} onHealthScan={(folder) => void browser.runtime.sendMessage({ type: 'queue-health-scan', input: { folderId: folder.id } })} reviewWorkspace={<ReviewWorkspace proposals={proposals} onApply={async (id, fields) => { await reviewService.applyProposal({ proposalId: id, fields }); await Promise.all([refresh(), refreshProposals()]); }} onReject={async (id) => { await reviewService.reject(id); await refreshProposals(); }} onRetry={async (proposal) => { await browser.runtime.sendMessage({ type: 'queue-analysis', input: { bookmarkId: proposal.bookmarkId } }); }}/>} notificationCenter={<NotificationCenter notifications={notifications} onMarkRead={async (id) => { await notificationRepository.markRead(id); await refreshNotifications(); }} onClear={async () => { await notificationRepository.clear(); await refreshNotifications(); }}/>} usageInsights={<UsageInsights nodes={nodes} aggregates={visitAggregates}/>} draftWorkspace={<NoteDraftWorkspace drafts={noteDrafts} onDelete={(id) => void noteDraftRepository.remove(id).then(() => noteDraftRepository.list().then(setNoteDrafts))}/>} />;
}
