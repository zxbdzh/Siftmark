...
export default function App() {
  ...
  const [context, setContext] = useState<{ node: BookmarkNode; x: number; y: number }>();
  ...
  const refresh = useCallback(async () => {
    ...
    setExpanded((current) =>
      current.size
        ? current
        : new Set(next.filter((node) => node.parentId === '0').map((node) => node.id))
    );
  }, [repository]);
  ...
  const descendants = (id: string): string[] =>
    (children.get(id) ?? []).flatMap((node) => [node.id, ...descendants(node.id)]);
  const toggleSelection = (node: BookmarkNode) => {
    const ids = isBookmark(node) ? [node.id] : [node.id, ...descendants(node.id)];
    setSelected((current) => {
      const next = new Set(current);
  ...
  const run = async (action: BatchAction) => {
    ...
    try {
      if (action === 'sort') {
        ...
        if (!selected.size || !globalThis.confirm(`确定删除已选择的 ${selected.size} 项？`)) return;
        const topLevel = [...selected].filter(
          (id) => !selected.has(nodes.find((node) => node.id === id)?.parentId ?? '')
        );
      ...
      setStatus(error instanceof Error ? error.message : `${actionLabel(action)}失败`);
    } finally {
      setBusy(undefined);
    }
  };
  ...
  const editNode = async (node: BookmarkNode) => {
    ...
    if (title && title !== node.title) await repository.update(node.id, { title });
  ...
  const renderNode = (node: BookmarkNode, depth: number): React.ReactNode => {
    ...
    const matches = !query || `${node.title} ${node.url ?? ''}`.toLocaleLowerCase().includes(query.toLocaleLowerCase());
    const descendantMatches = folder && childRows.some((child) => nodeMatches(child, query, children));
    ...
    return (
      <div key={node.id}>
        <div
          ...
          onDragOver={(event) => { if (folder) event.preventDefault(); }}
          onDrop={(event) => {
            if (!folder) return;
        ...
        {folder && open ? childRows.map((child) => renderNode(child, depth + 1)) : null}
      </div>
    );
  };

  return (
    <main className="manager-page" onClick={() => setContext(undefined)}>
      <header className="manager-topbar"><strong className="brand-type"><Sparkles size={18}/>Siftmark <span>· 书签树</span></strong><button type="button" onClick={() => void browser.runtime.openOptionsPage()}><Settings size={17}/>设置</button></header>
      <div className="manager-workspace">
        <section className="tree-panel">
          <div className="tree-search"><Search size={17}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索书签…"/>{query ? <button type="button" aria-label="清除搜索" onClick={() => setQuery('')}><X size={15}/></button> : null}</div>
          <div className="tree-scroll">{visibleRoots.map((node) => renderNode(node, 0))}</div>
        </section>
        <aside className="bulk-toolbar">
          <div className="selection-count">已选 <strong>{selectedBookmarks.length}</strong> 个书签</div>
          <button type="button" onClick={() => setSelected(new Set(bookmarks.map((node) => node.id)))}>全选书签</button>
          <button type="button" onClick={() => setSelected(new Set())}>清空选择</button>
          <hr/>
          <button type="button" onClick={() => void browser.tabs.create({ url: `${browser.runtime.getURL('/options.html')}#backup` })}><Save/>备份当前书签</button>
          <button type="button" disabled={Boolean(busy)} onClick={() => void run('sort')}><ArrowDownAZ/>批量排序</button>
          <button type="button" disabled={Boolean(busy) || !selectedBookmarks.length} onClick={() => void run('rename')}><Pencil/>批量改名</button>
          <button type="button" disabled={Boolean(busy) || !selectedBookmarks.length} onClick={() => void run('classify')}><FolderInput/>批量归类</button>
          <button type="button" disabled={Boolean(busy) || !selectedBookmarks.length} onClick={() => void run('health')}><CheckCircle2/>检测失效</button>
          <button type="button" className="danger" disabled={Boolean(busy) || !selected.size} onClick={() => void run('delete')}><Trash2/>批量删除</button>
          <output>{busy ? `${actionLabel(busy)}处理中…` : status}</output>
        </aside>
      </div>
      {context ? <menu className="tree-context-menu" style={{ left: context.x, top: context.y }} onClick={(event) => event.stopPropagation()}><button type="button" onClick={() => void editNode(context.node)}><Pencil/>重命名</button><button type="button" onClick={() => void createFolder(context.node)}><Folder/>新建文件夹</button><button type="button" className="danger" onClick={() => { setSelected(new Set([context.node.id])); setContext(undefined); }}><Trash2/>选择后删除</button></menu> : null}
    </main>
  );
}

function actionLabel(action: BatchAction): string {
  return { sort: '排序', rename: 'AI 改名', classify: 'AI 归类', health: '失效检测', delete: '删除' }[action];
}
function safeDomain(url: string): string { try { return new URL(url).hostname; } catch { return url; } }
function domainInitial(url?: string): string { return url ? (safeDomain(url)[0] ?? '•').toUpperCase() : '•'; }
function nodeMatches(node: BookmarkNode, query: string, children: Map<string, BookmarkNode[]>): boolean {
  if (!query) return true;
  if (`${node.title} ${node.url ?? ''}`.toLocaleLowerCase().includes(query.toLocaleLowerCase())) return true;
  return (children.get(node.id) ?? []).some((child) => nodeMatches(child, query, children));
}
